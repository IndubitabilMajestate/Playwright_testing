...
def Task4():
    with sync_playwright() as playwright:
        page = navigateToCategory(playwright.chromium.launch(headless=False).new_page(),'Elements','Web Tables')

        locator = Utils(page)
        webtable_locator = Webtable(locator.page)
        # Read webtable test
        table = webtable_locator.readTableData(".//div[@class='web-tables-wrapper']")
        print(table)
        ...
        form_locator = Form(locator.page)
        ...
        form_locator.fillForm("",new_data)
        locator.clickButton("//button[text()='Submit']")
        table = webtable_locator.readTableData(".//div[@class='web-tables-wrapper']")
